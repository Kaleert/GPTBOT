#мне нужно чтобы если в первой части ответа были открывающие код скобки, но нет закрывающих, то система сама адекватно (не на полуслове, а псоле конца строки, например, закрывала код и снова открывала их в следующем сообщении указав langname и так до тех пор пока в сообщении не найдет закрывающие кавычки
import mistune
import asyncio
import logging
import traceback
...
async def send_message_with_retry(bot, chat_id, text, parse_mode=ParseMode.HTML): #Изменено на HTML
    max_retries = 5
    text_chunks = split_code_blocks(text, 3000, "/")  # Передаем разделитель
    ...
    for chunk in text_chunks:
        for attempt in range(1, max_retries + 1):
            ...
            logging.error(f"Не удалось отправить сообщение после нескольких попыток: {chunk}")

def split_code_blocks(text, max_length, delimiter="/"):
    ...
    for i, part in enumerate(parts):
        if len(current_chunk) + len(part) <= max_length:
            current_chunk += part
            if i < len(parts) - 1:  # Добавляем разделитель, если это не последняя часть
...
async def generate_response(user_id, user_message, bot: Bot, client: OpenAI):
    try:
        ...
        print(f"Ответ нейросети(no-format): {response_message}")
        # Санитация Markdown (удаление тегов)
        response_message = re.sub(r'<[^>]+>', '', response_message)  # Удаляет все HTML-теги
        response_message = response_message.strip()
        print(f"Ответ нейросети(format): {response_message}")
        MAX_MESSAGE_LENGTH = 4096  # Максимальная длина сообщения Telegram
        response_parts = split_code_blocks(response_message, MAX_MESSAGE_LENGTH)  # Разбиваем сообщение

        for part in response_parts:
            await send_message_with_retry(bot, user_id, part, parse_mode="Markdown")  # Отправляем каждую часть
